#!/usr/bin/env python3
"""Full-scale PGO benchmark: certifiable GNC vs. vanilla GTSAM GNC.

Runs the Monte Carlo grid:

    methods   = {ours, vanilla}      (cert+GNC vs gtsam::GncOptimizer)
    inits     = {random, odom}       (random Pose2 vs VERTEX_SE2 from g2o)
    rates     = {10, 20, 30}         (outlier %)
    trials    = N (default 10)       (independent outlier draw + init seed)

Each trial:
  1. Inject outliers into the clean intel.g2o with seed=<trial>.
     File written into <out_dir>/data/intel_outl{rate}_seed{trial}.g2o.
  2. Run each (method, init) combination on that file.
  3. Parse <stem>_summary.csv to capture ATE, wall-clock, and (for
     certifiable runs only) staircase aggregates.

Outputs:
  <out_dir>/data/             outlier-injected datasets, one per trial
  <out_dir>/runs/             per-run artifacts (TUM trajectory,
                              iter log, weights, summary.csv) for all
                              N_methods × N_inits × N_rates × N_trials runs
  <out_dir>/per_run.csv       one row per individual run
  <out_dir>/summary.csv       one row per (method, init, rate) aggregate
                              (mean/std of ATE, time, GNC iters, max rank)

Usage:
    python scripts/run_benchmark_pgo.py \\
        --input data/pgo/intel.g2o \\
...
def sh(cmd, label, log_path=None):
    """Run a shell command, capturing output; return (rc, seconds, output)."""
    t0 = time.time()
...
def inject(input_g2o, output_g2o, pct, seed):
...
def main():
    ...
    ap.add_argument("--input",   required=True, help="Clean source g2o (e.g. data/pgo/intel.g2o)")
    ap.add_argument("--gt",      required=True, help="Ground-truth TUM trajectory")
    ap.add_argument("--out_dir", default=os.path.join(ROOT, "results", "benchmark_intel"))
    ...
    per_run_rows = []
    aggregated = {}  # (method, init, rate) -> list of dicts
    ...
    for rate in args.rates:
        for trial in range(args.num_trials):
            ...
            data_file = os.path.join(data_dir, f"intel_outl{rate}_seed{seed}.g2o")
            if not os.path.exists(data_file):
                inject(args.input, data_file, rate, seed)
            for method in args.methods:
                for init in args.inits:
                    ...
                    if method == "ours":
                        cmd = [
                            os.path.join(BIN, "Certifiable_GNC_PGO_example"),
                            "--d=2", "--p=2",
                            "--input_dir=" + data_file,
                            "--output_dir=" + stem,
                            "--init_type=" + init,
                        ...
                        cmd = [
                            os.path.join(BIN, "Vanilla_GNC_PGO_example"),
                            "--input_dir=" + data_file,
                            "--output_dir=" + stem,
                            "--init_type=" + init,
                            f"--seed={seed}",
    ...
    with open(summary_csv, "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(agg_cols)
        # Sort for stable order in the file: method, init, rate
